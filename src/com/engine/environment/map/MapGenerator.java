...
public final class MapGenerator {
	...
	private static float[][] diffuseHeight(float[][] pointData) {
		...
		for (int x = 2; x < width - 2; x++) {
			for (int y = 2; y < height - 2; y++) {
				...
				pointData[x][y] = (0.5f * pointData[x][y]) + (0.5f * (tHeight / t));
			}
		}
	...
	// Frequency 0.0 - 1.0
	private static float[][] generateHills(float[][] pointData, float frequency, int minHeight, int maxHeight,
			float scale) {
		int width = pointData.length;
		int height = pointData[0].length;
		for (int x = 0; x < width; x++) {
			for (int y = 0; y < height; y++) {
				if (x <= 1 || y <= 1 || x >= width - 2 || y >= height - 2)
					continue;
				pointData[x][y] = (float) (Math.random() * Integer.MAX_VALUE < frequency * Integer.MAX_VALUE
						? ((Math.random() * (maxHeight - minHeight)) + minHeight) / scale : pointData[x][y]);
			}
		}
		return pointData;
	}
		...
		for (int x = 1; x < width; x++) {
			for (int y = 1; y < height; y++) {
				faceList.add(new Face(
						new Point3D[] { new Point3D(((x - (width / 2)) - 1) * scale, ((y - (height / 2)) - 1) * scale,
								pointData[x - 1][y - 1]),
						new Point3D((x - (width / 2)) * scale, ((y - (height / 2)) - 1) * scale, pointData[x][y - 1]),
						new Point3D((x - (width / 2)) * scale, (y - (height / 2)) * scale, pointData[x][y]) },
						Color.WHITE));
				faceList.add(
						new Face(new Point3D[] {
								new Point3D((x - (width / 2)) * scale, (y - (height / 2)) * scale, pointData[x][y]),
								new Point3D(((x - (width / 2)) - 1) * scale, (y - (height / 2)) * scale,
										pointData[x - 1][y]),
						new Point3D(((x - (width / 2)) - 1) * scale, ((y - (height / 2)) - 1) * scale,
								pointData[x - 1][y - 1]) }, Color.WHITE));
			}
		}
